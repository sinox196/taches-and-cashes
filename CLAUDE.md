...
**La console plateforme (`PlatformUsersModal.tsx`) rend la même information sans attendre un mail.** Elle reçoit désormais `plan` depuis la ligne entreprise déjà chargée par `PlatformAdmin.tsx`, et calcule côté client — sur les mêmes règles que le serveur, `baseSeats`/`pricePerExtraUserDT` de `planMeta(plan)` — combien de comptes sont inclus et lesquels sont en supplément : les comptes back-office triés par `createdAt` (une chaîne vide, pour un compte antérieur à ce champ, se trie avant tout le reste — c'est correct ici, puisqu'un tel compte a nécessairement été créé le premier), tout ce qui dépasse `baseSeats` porte le badge « Siège supplémentaire ». Chaque ligne affiche aussi sa date de création (« Créé le JJ/MM/AAAA », ou « Date de création inconnue » pour un compte hérité) — c'est le « geddch 3ml men compte w date de creation » demandé : combien de comptes, et depuis quand chacun existe, visible depuis la console sans dépendre du mail.
