...
**That phone-mockup illustration was itself replaced with a real photo, at the user's explicit request** — a stock photo of a "gestionnaire comptable" at her desk, a MacBook plus an external monitor showing a dashboard mock-up, set against a city-view office. The file lives at `public/landing/hero-photo.jpg` (1584×672, referenced by a plain `/landing/hero-photo.jpg` path — no base64 in the bundle, same reasoning as the seeded liens-utiles logos) and is drawn with `object-fit: cover` inside a `600×430` rounded card; `objectPosition: '68% 42%'` was picked by eye to keep the model and the monitor centred in frame while still showing a slice of the photo's own decorative finance-icon backdrop on the left, since that backdrop already echoes the app's own subject matter. The hand-drawn HTML "Tableau de bord" mock-up card (sidebar rail, five KPI tiles, a bar chart, a two-row client table) that used to sit here is gone outright, not kept as a fallback — the photo's own monitor already shows an equivalent dashboard, so keeping both would have been the same idea told twice.
