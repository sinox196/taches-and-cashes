...
**A `lg`-only accent layer scatters four of the original floating stat cards (`landingFloatA`–`D`) over the photo**, positioned by `right-%`/`top`/`bottom` within the same z-10 layer as the copy (not inside the masked photo `<div>`, so the mask never touches their own crisp edges) rather than reusing the boxed version's tight corner-anchored positions — the photo is now much wider than a 600px card, so the cards are spread further apart to match, clustered loosely around where the laptop and monitor sit in the photo rather than pinned to a card's four corners. The fifth original card ("Nouvelle tâche assignée") and its `landingPulseDot` animation were dropped rather than carried over — four cards already read as a full scene against a wide photo; a fifth started to clutter it. A fifth element, a white "Tâches & Cash" pill badge centred over the photo, was added in the same layer and then **removed at the user's explicit follow-up request**: once the crop was retuned to show the monitor's own "Tableau de bord" mock-up in full (see `objectPosition` below), the badge sat directly on top of it, covering the exact detail the retune existed to reveal.

The floating stat cards around it (Temps travaillé, Audit client, Équipe, Flux de trésorerie — originally five, including a "Nouvelle tâche assignée" card dropped in the full-bleed rewrite above) read as feature call-outs layered over *any* hero visual, not as a literal transcription of what the mock-up card used to show, so swapping the card under them for a photo cost nothing.
