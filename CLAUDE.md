...
**A group header is deliberately louder than the nav labels under it** (`text-[12px] font-extrabold text-white/70` plus a turquoise `•` bullet), not a subtler `10px`/dimmed treatment — the first pass read as too close in weight to the items it was heading, the exact opposite of what a section header needs to do. The bullet is a plain `<span className="text-turquoise">•</span>`, the same accent colour as the app wordmark's `&` in the logo above it, not a new colour introduced just for this.
