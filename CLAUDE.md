...
- **Cash keeps a single accent colour on its tab bar's active state** (`border-blue-600 text-blue-600`) — Facturation, Règlements clients and Brouillard de caisse are views onto the same underlying `invoices`/journal data, not separate identities. **Tâches and RH go the other way: every sub-tab carries its own colour**, one per subview rather than one for the whole section — see below.
