...
**One deliberate exception to "never reassigned on edit": `POST /api/invoices/:id/renumber` rebases the legal sequence's starting point**, for a cabinet moving into the app mid-year with invoices already issued on paper or in a previous tool. It is restricted to *only* the year's invoice n° 1 (`existing.number !== '0001'` is rejected outright), and only while it is still the sole legal invoice issued for that year — the moment a second one exists, the ordering rule above would make renumbering the first one retroactively break the sequence, so it is refused the same way any other renumbering is. Renumbering both moves the invoice's own `number` and calls the dedicated `Database.setInvoiceCounter(companyId, counter, year)` so the *next* legal invoice continues right after it — **never** through `updateSettings()` for this: under Postgres it writes `invoiceCounter` into its own dedicated column but would route `invoiceCounterYear` into the JSONB `data` blob, which `nextInvoiceNumber()` never reads (it reads the real `invoice_counter_year` column), so the year would silently fail to move and the next invoice would reset to `0001` instead of continuing the sequence. The button (`Hash` icon) only renders next to a document that qualifies — `documentKind === 'FACTURE_LEGALE' && number === '0001'` — and disappears the moment it no longer does, immediately after a successful renumber.
