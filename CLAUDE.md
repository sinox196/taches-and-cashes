...
**A `lg`-only accent layer scatters four of the original floating stat cards (`landingFloatA`–`D`) plus the brand badge over the photo**, positioned by `right-%`/`top`/`bottom` within the same z-10 layer as the copy (not inside the masked photo `<div>`, so the mask never touches their own crisp edges) rather than reusing the boxed version's tight corner-anchored positions — the photo is now much wider than a 600px card, so the cards are spread further apart to match, clustered loosely around where the laptop and monitor sit in the photo rather than pinned to a card's four corners. The fifth original card ("Nouvelle tâche assignée") and its `landingPulseDot` animation were dropped rather than carried over — four cards already read as a full scene against a wide photo; a fifth started to clutter it.

The five floating stat cards around it (Temps travaillé, Audit client, Équipe, Flux de trésorerie, Nouvelle tâche assignée — the `landingFloatA`–`E`/`landingPulseDot` animations) are untouched: they read as feature call-outs layered over *any* hero visual, not as a literal transcription of what the mock-up card used to show, so swapping the card under them for a photo cost nothing. A small brand badge (`<Logo size={18} />` + "Tâches & Cash" in a white pill) sits at the photo's bottom-right corner — the one corner none of the five floating cards already claims — so a real stock photo still reads as *this* product's marketing rather than a generic one; a soft bottom-edge navy gradient wash under the photo keeps that pill legible regardless of what the crop happens to put behind it. The card's border is a 1.5px navy→turquoise diagonal gradient (padding on the wrapper, not a CSS `border-color`) rather than a flat rule, because a flat border read as an afterthought against a photograph in a way it never did against a hand-drawn card — the same "designed, not bolted on" reasoning `TONES` documents for the pricing cards above.
