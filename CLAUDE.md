...
**Brouillard de caisse** ([CashJournal.tsx](src/components/cash/CashJournal.tsx), second tab of Cash) is the cabinet's cash daybook, modelled on their own sheet — one row per movement: `entree` (money in) or `sortie` (money out), plus date, **mois**, libellé, client, **catégorie**, mode de règlement and n° de pièce. `category` is **free text with suggestions**, not an enum: their list is long and still growing (Transport, Loyer, Femme de ménage, STEG, SONEDE, OOREDOO, …), and a closed list would mean a code change every time it grows. **Mois is derived from the date**, never stored, and doubles as the month filter — the sheet is read one month at a time.
