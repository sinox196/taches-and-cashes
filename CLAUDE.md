...
**"Facturation & Trésorerie" (25 characters, longer than every other nav label) was clipped to "Facturation & Trésor…" in the sidebar's fixed 212px rail.** Every `mainNavItems` row shared one `truncate` (single-line, ellipsis) treatment on both its flex container and its label `<span>` — safe while every label happened to fit on one line, but this one no longer did. Widening the whole rail for one label would have reflowed every page next to it; the fix instead lets the label wrap (`items-start` on the row instead of `items-center`, `truncate` dropped, `leading-snug` on the span) so this row grows to two lines while every shorter label keeps rendering identically on one. The icon carries `mt-px` to stay pinned to the first line rather than centering against the two-line block.
