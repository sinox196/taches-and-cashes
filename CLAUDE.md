...
**The overtime alert fires on the task's own duration — at 2h, then 4h, 6h, …** ([App.tsx](src/App.tsx)). Once a running task crosses a 2h milestone it prompts "Toujours sur cette tâche ?" with three answers — continuer, mettre en pause, arrêter. A prior version auto-paused the task after a 2-minute grace period if left unanswered; **removed at the user's explicit request** — a task must keep running for as long as nobody has actually told it to do otherwise. The popup now has no deadline of its own: it waits until one of the three buttons is pressed, and changes nothing on the entry until then.
...
**Une cellule réglée sur un statut « payé » (couleur `done` — aujourd'hui « Oui », la seule valeur semée dans cette couleur) demande un détail de règlement avant de s'enregistrer.** Cliquer une telle valeur dans le menu de la cellule n'écrit rien tout de suite : un petit panneau s'ouvre à la place (même idiome que l'éditeur de colonne — un popover positionné, pas une vraie modale), demandant **N° Quittance** et **Montant (DT)**, pré-rempli avec ce que la cellule porte déjà pour permettre une correction sans retaper. La cellule ne change qu'à l'« Enregistrer » de ce panneau ; « Annuler » (ou un clic en dehors) la laisse exactement comme avant. `pickStatus()` dans [EcheancesGrid.tsx](src/components/resources/EcheancesGrid.tsx) est le point d'aiguillage : il teste la **couleur** de l'option choisie, jamais son libellé « Oui » en dur — la même raison que `COLOR_TOKENS` existe déjà pour tout le reste de cet écran, puisque la valeur est admin-éditable (voir « The status vocabulary itself... » plus haut) et peut être renommée sans que ce comportement s'en détache. Un statut d'une autre couleur (ou « Vide ») s'applique toujours en un clic, sans détour par ce panneau.

`PUT /api/echeance-statuses` accepte donc en plus `quittanceNumber` (texte, 60 caractères) et `montant` (nombre ≥ 0, arrondi au millime comme le reste de l'argent dans l'app). **Les deux n'ont de sens que sur un statut réglé, et la route les redérive entièrement de la requête en cours plutôt que de fusionner sur ce qui était déjà stocké** : choisir un autre statut, ou revenir à « Vide », les remet toujours à `null`, même quand le corps de la requête ne les mentionne pas. Une fusion partielle aurait laissé une ancienne quittance traîner sur une cellule qui ne dit plus « Oui », prête à ressurgir intacte le jour où une valeur de statut change de couleur (recolorer « DEFAUT » en `done`, par exemple) — le même raisonnement qui fait déjà retomber `idleMs`/le type de poste à `null` dès qu'un utilisateur passe INACTIF en présence : une donnée périmée ne doit jamais se lire comme à jour.

**Le détail voyage jusqu'au portail et déclenche la notification `PORTAL_ECHEANCE` existante, sans nouveau mécanisme.** `notifyEcheanceChange()` prend `quittanceNumber`/`montant` en plus et les glisse dans le corps du message (« … est maintenant « Oui » (Quittance N° 2026-0451, 126 TND). ») quand ils sont présents — c'est la même notification que pour tout autre changement de statut, juste enrichie ; aucun type de notification de plus. `GET /api/portal/echeances` renvoie les deux champs sur chaque statut, et `GET /api/echeance-statuses` (admin) les porte déjà de lui-même puisque `createEcheanceStatus`/`updateEcheanceStatus` stockent la ligne telle quelle — aucune migration de schéma nécessaire, les deux moteurs étant document-shaped (voir « Two storage backends behind one interface »).

**L'affichage suit la même règle des deux côtés** : une cellule qui porte un détail montre son statut sur une ligne et « N°… · … DT » en plus petit juste en dessous — dans le tableau (`EcheancesGrid.tsx`, la cellule grandit d'elle-même via `min-h-7` plutôt qu'une hauteur fixe, donc seules les lignes concernées s'élargissent) et dans « Calendrier par client » (même écran, et sa contrepartie portail `EcheancesView` dans [ClientPortal.tsx](src/pages/ClientPortal.tsx), en lecture seule — le portail n'a jamais eu de route d'écriture sur les échéances). Les trois copies de ce petit rendu (`detailLine()` deux fois côté admin — tableau et calendrier partagent la même fonction — et une fois côté portail) restent volontairement séparées plutôt que partagées entre deux fichiers qui n'ont par ailleurs aucune dépendance l'un envers l'autre, même raison que les tokens de couleur des échéances sont déjà dupliqués entre les deux écrans.

**Les trois panneaux flottants de cet écran (menu de cellule, éditeur de colonne, panneau Quittance) restent dans l'écran, quel que soit l'endroit cliqué.** Les trois se positionnaient en `position: fixed` au point de clic sans jamais vérifier s'ils débordaient — un clic sur une des dernières lignes visibles de la liste, ou sur une des dernières colonnes visibles après un défilement horizontal, poussait le panneau sous le bas ou au-delà du bord droit de l'écran : ses options existaient toujours dans le DOM, invisibles ou coupées, sans qu'aucun défilement ne les ramène puisque `position: fixed` échappe justement au défilement du tableau. C'était le bug remonté (« les choix ne s'affichent pas, ou s'affichent incomplets », sur une ligne en fin de liste ou une colonne au milieu/à droite du tableau). `useClampToViewport()` — un petit hook partagé par les trois refs (`menuRef`/`editRef`/`quittanceRef`) — mesure le panneau une fois monté (`useLayoutEffect`, donc avant la peinture, sans clignotement) et recale `left`/`top` pour qu'il tienne entièrement dans le viewport, avec une marge de 8px ; sans tableau de dépendances, il se réajuste à chaque rendu — bon marché pour un DOM de cette taille, et nécessaire puisque le panneau peut changer de forme en restant ouvert (« Ajouter une valeur », l'édition d'une valeur…). Même idée que le correctif déjà en place pour le panneau de `SearchableSelect` (`ResizeObserver`/`panelEl`, voir « Missions and types de tâches » plus haut), en plus simple ici puisque la taille ne varie pas en continu, un seul ajustement après montage suffit.
...
**Complet porte désormais une remise de lancement affichée, à la demande explicite de l'utilisateur.** Le tarif catalogue est 25 DT/mois par utilisateur ; `priceDT`/`pricePerExtraUserDT` restent à **15 DT** — c'est le tarif déjà remisé, celui réellement facturé, celui que le mail de RIB et la confirmation de paiement annoncent — donc rien ne change dans `planPriceForSeats()` ni dans aucun des quatre endroits qui l'appellent. Seul `PlanMeta.launchDiscountPercent` (`40` sur `COMPLET`) est nouveau, et il n'entre dans **aucun** calcul de montant dû : il ne sert qu'à reconstituer le prix barré sur la page Tarifs, via `planListPriceForSeats(meta, seats)` — `planPriceForSeats(meta, seats) / (1 − launchDiscountPercent / 100)`, dans plans.ts juste après `planPriceForSeats()`. Dérivé plutôt que stocké comme un second prix figé : diviser le tarif réel par `(1 − remise)` ne peut jamais diverger de ce qui est effectivement facturé, quand bien même `priceDT`/`pricePerExtraUserDT` changeraient sans qu'on pense à toucher un second champ ailleurs — le même réflexe que `computeInvoiceTotals()`/`planPriceForSeats()` elles-mêmes évitent déjà une duplication de calcul. La carte Complet ([Landing.tsx](src/pages/Landing.tsx)) affiche donc un badge « OFFRE DE LANCEMENT · −40 % » (libellé choisi côté rédaction — l'utilisateur n'avait donné qu'un pourcentage et un intitulé provisoire à reformuler) au-dessus du prix, puis le tarif catalogue barré à côté du tarif réel (`25 DT` barré, `15 DT` en grand à 1 utilisateur ; `125 DT` barré, `75 DT` à 5, le curseur de sièges faisant scaler les deux ensemble puisque les deux sont un tarif plat par tête). Freelancer n'a pas de `launchDiscountPercent` et ne montre donc jamais de prix barré — la remise est propre à Complet, pas une règle générale du catalogue. La modale d'inscription (`RequestAccessModal.tsx`) n'a délibérément pas été touchée : elle continue de n'afficher que le tarif réel, ce que demande la portée de la modification.
...
**« Exporter PDF », dans la barre de filtre, exporte un instantané du tableau de bord pour exactement la période que le filtre affiche à l'écran** — un jour précis, « Aujourd'hui », un mois, une année ou une plage Du/Au libre. [dashboardPdf.ts](src/components/dashboard/dashboardPdf.ts) est le rendu, dessiné avec les mêmes primitives texte jsPDF que `clientReportPdf.ts`/`invoicePdf.ts`/`payslipPdf.ts` — du vrai texte vectoriel, pas une capture d'écran — et suit leurs mêmes conventions (palette `INK`/`MUTED`/`LINE`/`ACCENT`, `sectionTitle()` à onglet turquoise, pagination par `ensureSpace()`/re-dessin de l'en-tête de tableau après `doc.addPage()`). Contrairement à `invoicePdf.ts`, pas de lettre à en-tête d'entreprise ni de dance impression-par-iframe : c'est un rapport interne, `doc.save()` suffit, même choix que `clientReportPdf.ts`.

**Le PDF ne redécide aucun périmètre — il affiche ce que `exec`/`stats` contiennent déjà.** Les deux réponses sont déjà filtrées côté serveur pour le rôle appelant (montants absents pour un SUPERVISEUR ou un filtre collaborateur actif, `exec.clients`/`exec.concentration` absents pour un non-ADMIN) exactement comme le reste de l'écran ; le renderer se contente de vérifier `!= null`/la présence des champs plutôt que de relire `user.role`, la même garde que suit déjà `AIDashboardSummary`. Le libellé de période (`periodLabel`, calculé dans `AdminDashboard.tsx` à partir de `monthFilter`/`yearFilter`/`startDate`/`endDate` — pas un nouveau champ serveur) suit la même hiérarchie que le filtre lui-même : mois choisi, sinon année choisie, sinon un jour unique (« Aujourd'hui (JJ/MM/AAAA) » ou une date passée en clair), sinon une plage « Du … au … » — et sert à la fois le titre du PDF et le nom du fichier téléchargé.

Le rapport reprend les sections de l'écran dans le même ordre — bandeau exécutif, indicateurs globaux, alertes, rentabilité du portefeuille (top 15 par honoraires) + concentration, missions & types de tâche (top 15 par heures), performance des collaborateurs — chacune omise plutôt qu'affichée vide quand la donnée source n'existe pas (pas d'appel `/api/dashboard/executive` réussi, aucun client facturé, aucune mission…), même règle que les cartes de l'écran qui se replient ou disparaissent dans ce cas.

**Les trois tableaux du PDF reprennent toutes les colonnes de leur équivalent à l'écran, pas un sous-ensemble.** Une première version n'en gardait qu'une poignée par table — « Rentabilité du portefeuille » disait Client/Heures/Honoraires/Marge/Taux quand [ClientProfitability.tsx](src/components/dashboard/ClientProfitability.tsx) en montre huit (+ Coût du temps, Hon./h, Reste dû), « Missions & types de tâche » en tait deux (Coût, Durée moy.), et « Performance des collaborateurs » n'en gardait que 6 sur les 14 d'[EmployeeTable.tsx](src/components/dashboard/EmployeeTable.tsx) — ce qui se lisait comme un export incomplet plutôt que comme un résumé délibéré. Les trois tables portent désormais exactement les mêmes colonnes, dans le même ordre, que leur écran — Coût employeur `EmployeeTable.tsx` compris, qui reste conditionné à `isAdmin` (`rows.some(r => r.cout)` reproduit la même garde).

**La table Performance des collaborateurs (jusqu'à 14 colonnes) bascule en A4 paysage plutôt que de tasser ses colonnes en portrait.** `buildDashboardPdf()` garde l'orientation de page dans des variables mutables (`orientation`/`right`/`bottom`, plus `goLandscape()`) que `rule()`/`ensureSpace()`/`drawTable()`/`drawTableHeader()` lisent à chaque appel plutôt que de fermer sur des constantes — la table est la dernière section du rapport, donc rien n'a besoin de repasser en portrait après elle. Le pied de page (numérotation) lit `doc.internal.pageSize.getWidth()/getHeight()` **par page** plutôt qu'une constante portrait, sinon le numéro d'une page paysage s'imprimait hors de la feuille (297×210 mm, pas 210×280).

**Les largeurs de colonnes sont mesurées, jamais devinées.** Une première passe avait fixé une largeur en dur par colonne (à l'œil, en comptant les caractères d'un libellé) — juste assez large pour « COÛT » mais pas pour « COÛT EMPLOYEUR », « CONGÉS PRIS » ou « SOLDE CONGÉS », qui se recouvraient dans l'en-tête (repéré sur `Tableau de bord Direction`, page paysage : les trois libellés s'enchaînaient sans espace). `fitColumns()` mesure chaque colonne via `doc.getTextWidth()`, dans la police et la taille réellement utilisées à l'affichage (en-tête **et** corps — la valeur la plus longue des deux gagne), et n'a donc plus besoin qu'un `minW` optionnel pour les colonnes à contenu toujours très court. Si la somme dépasse la largeur disponible (rare, avec l'espace donné par le paysage), tout le tableau est mis à l'échelle plutôt que de laisser une colonne déborder de la page.

**Les mini-cartes de synthèse (`statRow()`, « Vue d'ensemble »/« Indicateurs globaux ») avaient deux bugs distincts qui se lisaient tous les deux comme des valeurs qui se chevauchent, et les deux n'apparaissaient qu'avec de vraies données — jamais avec les zéros d'un compte tout juste seedé, ce qui les a laissés passer au premier essai.**

- **La chaîne fr-FR groupe les milliers avec une espace fine insécable (U+202F), absente des polices standard de jsPDF.** Non normalisée, `975000` s'imprimait « 9 7 5 / 0 0 0 » — chaque caractère écarté par un glyphe de remplacement, exactement le piège que `money()` documente déjà pour `invoicePdf.ts`/`clientReportPdf.ts`, ici manqué à l'écriture de ce fichier. Le correctif est passé dans `nf()`, le formateur de base (`.replace(/[  ]/g, ' ')`) plutôt que dans chaque appelant, pour que `money()`/`pct()`/tout futur nombre en hérite d'un coup. Le même glyphe non supporté faussait aussi les mesures de largeur (`doc.getTextWidth()`/`splitTextToSize()` sous-estiment un caractère qu'elles ne savent pas peser), donc corriger `nf()` a aussi supprimé les débordements de colonne qui en découlaient — un seul correctif pour les deux symptômes.
- **`statRow()` concaténait une valeur et son détail sur une seule ligne** (« 92 % (capacité 176h00) », « 45 % (975 000 TND) ») sans aucune contrainte de largeur — invisible avec des zéros, ça débordait sur la carte voisine dès qu'un vrai chiffre entrait en jeu. `statRow()` prend maintenant `{label, value, foot?}` et dessine `value` en gras puis `foot` sur sa propre ligne plus petite, en dessous — la même charpente que la `Card` d'[ExecutiveBar.tsx](src/components/dashboard/ExecutiveBar.tsx) à l'écran (libellé, valeur, pied). Une garde de largeur mesurée reste en place sur les deux lignes (`wrap()` tronque à la première ligne plutôt que de déborder) pour le cas, désormais rare, où un chiffre serait malgré tout trop large.

**Deux sections manquaient purement et simplement, remontées après coup : les types de tâche sous chaque mission, et « Activité par client » (avec les tâches de chaque client).** La première version de « Missions & types de tâche » ne dessinait que la ligne mission — `taskTypes[]`, le détail que la carte déplie au clic à l'écran, n'était jamais lu. `drawRow()` (une seule ligne, colonnes + décalage optionnel — extrait de `drawTable()` pour être réutilisé) dessine maintenant une sous-ligne indentée par type de tâche, sur les **mêmes** colonnes MISSION/HEURES/TÂCHES/COÛT que la ligne mission (DURÉE MOY./COLLAB./CLIENTS n'existent qu'au niveau mission et restent vides sur ces lignes) — pas un second jeu de colonnes, la sous-ligne se contente d'un décalage `indent` et d'une encre plus discrète (`MUTED`, 6.5pt).

« Activité par client » n'existait pas du tout dans le rapport, alors que c'est l'un des deux blocs de « 04 · Clients & équipe » à l'écran (l'autre étant Performance des collaborateurs, déjà présent). Elle reprend les colonnes d'[ClientBreakdown.tsx](src/components/dashboard/ClientBreakdown.tsx) (Client, Tâches, Intervenants, Durée, Coût employeur ADMIN, puis le grand-livre — Solde antérieur/Montant de facture/Encaissements/Reste à payer — quand ces champs existent) et, sous chaque client, son propre tiroir « Tâches réalisées ». Neuf colonnes une fois le grand-livre présent a le même problème de largeur que la table Performance : la section bascule donc en paysage elle aussi (`goLandscape()`), sans quoi `fitColumns()` réduisait tout — y compris la colonne TÂCHES jusqu'à tronquer sa propre valeur (« 3 (3 » au lieu de « 3 (3 term.) »).

**Le tiroir « Tâches réalisées » par client n'est pas dans `exec`/`stats` — c'est un aller-retour réseau que l'écran ne fait qu'au clic** (`GET /api/kpi/client-tasks`, le même que `ClientBreakdown.tsx` appelle). `DashboardPdfInput` porte donc un `clientTasksByKey` optionnel, rempli par l'appelant avant de construire le PDF plutôt que par ce fichier — `dashboardPdf.ts` reste un pur renderer, aucun appel réseau à lui. `AdminDashboard.tsx`'s `handleExportPdf` est passé asynchrone pour ça : il récupère, en une salve (`Promise.all`), les tâches de chaque client dont `taskCount > 0` (les clients sans activité n'ont rien à demander), avec exactement les mêmes filtres (`startDate`/`endDate`/`filterUserIds`/`filterClientIds`) que le reste de l'écran — un bouton « Export en cours… » (`exportingPdf`, `Loader2` en rotation) couvre l'attente, désactivé le temps de la salve pour qu'un second clic n'en relance pas une deuxième. La colonne COÛT de ce sous-tableau ne s'affiche que si au moins une tâche renvoyée en porte un (`t.cost !== undefined`) — la même garde ADMIN que `/api/kpi/client-tasks` applique déjà côté serveur (`isAdminViewer`). Le plafond de 200 tâches par client que la route impose déjà se répercute tel quel dans le PDF : `truncated > 0` imprime une ligne « … et N tâche(s) supplémentaire(s) non affichée(s) » plutôt que de le taire.
...
**Le drill-down « Tâches réalisées » d'`ClientBreakdown.tsx` a désormais son propre tri, indépendant du tableau des clients au-dessus.** Chaque client ouvert affiche ses tâches (Date, Collaborateur, Activité, Mission, Type de tâche, Durée, Coût, Statut) ; les huit colonnes sont maintenant cliquables avec le même idiome `ArrowUpDown`/`ArrowUp`/`ArrowDown`, `detailSortField`/`detailSortDirection` étant partagé par tous les tiroirs ouverts à la fois plutôt qu'un état par client — la même simplification que fait déjà `sortField` pour le tableau extérieur. `Coût` est `null` pour une tâche sans taux configuré et va en fin de liste quel que soit le sens, même règle que partout ailleurs. `Date` compare `t.date` (un `DD/MM/YYYY` écrit par le serveur) réordonné en `YYYY-MM-DD` avant la comparaison de chaînes — le même idiome `toIsoDateKey` que `TimeTrackingTable.tsx`, pour ne jamais faire de round-trip par `Date`.

**Une ligne dépliée — mission dans `TaskIntelligence.tsx`, client dans `ClientBreakdown.tsx` — se colore pendant qu'elle est ouverte, et redevient neutre à la fermeture.** La ligne cliquée passe en `bg-blue-50/70` (et son survol en `bg-blue-50`), et son contenu déplié (les types de tâche d'une mission, le bloc Intervenants/Tâches réalisées d'un client) en `bg-blue-50/40` — un ton plus clair pour rester lisible sous le texte. C'est un simple ternaire sur `isOpen`/`open`, donc replier la ligne revient exactement à l'état neutre d'avant (`hover:bg-gray-50`) sans code séparé pour « réinitialiser » la couleur. Le bleu est choisi parce que c'est déjà l'accent générique de l'app (Cash-Facturation, l'icône de `EmployeeTasksModal.tsx`…), jamais un des jetons réservés `run`/`done`/`pause`/`late`/`admin`/`collab`.

**`EmployeeTasksModal.tsx` (« Clients traités », ouvert depuis une ligne du tableau de performance) affichait encore une colonne « Pôle » — restée en toutes lettres alors que `TaskIntelligence.tsx` et `ClientBreakdown.tsx` disent déjà « Mission » ailleurs sur le même tableau de bord.** Renommée en « Mission », et une colonne « Type de tâche » ajoutée juste à côté (`t.taskType`, déjà renvoyé par `/api/kpi/employee-tasks` mais jusque-là ignoré côté écran) — les deux informations qui identifient une tâche vont ensemble, comme dans les deux autres cartes.
...
**That phone-mockup illustration was itself replaced with a real photo, at the user's explicit request** — a stock photo of a "gestionnaire comptable" at her desk, a MacBook plus an external monitor showing a dashboard mock-up, set against a city-view office. The file lives at `public/landing/hero-photo.jpg` (1584×672, referenced by a plain `/landing/hero-photo.jpg` path — no base64 in the bundle, same reasoning as the seeded liens-utiles logos). The hand-drawn HTML "Tableau de bord" mock-up card (sidebar rail, five KPI tiles, a bar chart, a two-row client table) that used to sit here is gone outright, not kept as a fallback — the photo's own monitor already shows an equivalent dashboard, so keeping both would have been the same idea told twice.

**The photo went through two treatments before landing on the one described below, both superseded at the user's explicit follow-up requests.** A first pass boxed it in a rounded, bordered card beside the copy (`600×430`, `object-fit: cover`, a gradient frame) — replaced because the user wanted the photo full-width, edge to edge ("l cover image lzmha tkoun 3la kamel largeur mta3 l ecran… ma t7othch f box w tkhali l be9i blanc" — the cover has to span the full screen width, including behind the headline and subtitle, not sit in a box with the rest left blank). A second pass went full-bleed but with a dark navy scrim over the whole photo and white copy — replaced again because the user then shared a reference screenshot ("tkoun kif heka l cover image") showing the photo blending into the page's own **light** background via a soft fade, with the original navy-on-white copy sitting beside it, not white text on a dark photo.

**The current version, matching that reference, pins the photo to the section's right edge and fades it to transparent on its own left edge with a CSS mask** (`WebkitMaskImage`/`maskImage: linear-gradient(90deg, transparent 0%, #000 32%)`), rather than a hard box edge or a dark scrim — where the mask goes transparent, the section's ordinary `linear-gradient(180deg,#FBFCFD,#F2F4F7)` background shows through, so the photo reads as bleeding into the page rather than sitting on top of it. The copy (badge, h1, subtitle, buttons) is completely unchanged from before either photo treatment — still `text-navy`/`text-[#3D4655]`/`text-[#5B6472]` on the light background, still the navy-primary/white-secondary button pair — because the light-fade approach means it never needed to change color for contrast in the first place; only the visual next to it did. `objectPosition: '86% 38%'` (desktop) / `'62% 35%'` (mobile band) keeps the model and the monitor mock-up in frame — the desktop value moved from an initial `58%` at the user's explicit follow-up request ("scale it at the left to see the dashboard of our platform in the image"): at `58%` the crop window (the container is only ~58%/54% of the viewport width, and `object-fit: cover` scales the 2.357:1 source up to fill its height, so only a narrow horizontal slice of the source is ever visible) sat too far left, cropping out almost the entire monitor — the dashboard mock-up drawn on it, the whole reason the photo is on-brand rather than a generic stock shot, was reduced to a one-pixel sliver clipped at the viewport's own right edge. Higher `objectPosition` X shifts that slice rightward across the source image; `86%` was tuned by screenshot at 1440px and re-checked at 1920px until the monitor's "Tableau de bord" title, its five KPI tiles and the bar chart beneath them all sit fully inside the visible crop rather than running off the edge.

**The side-by-side treatment only activates at `lg` (1024px) and up, not `sm` (640px).** The photo layer and its floating cards are `hidden lg:block`; below that, a second, independent `<img>` renders as its own full-width band under the copy (`lg:hidden`, masked on its *top* edge instead of its left — `linear-gradient(180deg, transparent 0%, #000 22%)` — same fade-into-the-page idea, just the other axis, since there's no side to blend into once the layout stacks). This was tightened from an initial `sm:block` cutover after checking a tablet width (820px): at `sm`, the photo already claims a fixed 62% of the *full viewport* while the copy's own 560px-wide column, unrelated to the photo's width, kept extending into the photo's fully-opaque area past the mask's fade zone — text sat directly on the unfaded photo with no scrim to help it, unreadable in patches. Moving the cutover to `lg` gives the copy column enough spare viewport width before the photo ever starts that the two no longer compete for the same pixels.

**A `lg`-only accent layer once scattered four of the original floating stat cards (`landingFloatA`–`D`) over the photo, plus a white "Tâches & Cash" pill badge — both are gone now, removed in two separate follow-up requests.** The layer originally positioned the cards by `right-%`/`top`/`bottom` within the same z-10 layer as the copy (not inside the masked photo `<div>`, so the mask never touched their own crisp edges), spread apart to match a photo much wider than the old 600px card, clustered loosely around where the laptop and monitor sit rather than pinned to a card's four corners — the fifth original card ("Nouvelle tâche assignée") and its `landingPulseDot` animation had already been dropped rather than carried over, since four cards already read as a full scene against a wide photo and a fifth started to clutter it. The badge was added in the same layer and then removed once the crop was retuned to show the monitor's own "Tableau de bord" mock-up in full (see `objectPosition` below) — it sat directly on top of that detail. The remaining four cards were then removed too, at the user's own explicit follow-up request ("remove those labels also 08h42 equipe 84% audit client encours +12450dt") — the whole `hidden lg:block pointer-events-none` block that held them is gone outright, not hidden: the photo now shows only the real dashboard mock-up drawn on the monitor, with no invented UI-card overlay competing with it.

The floating stat cards that used to sit around it (Temps travaillé, Audit client, Équipe, Flux de trésorerie, and originally a fifth "Nouvelle tâche assignée") had read as feature call-outs layered over *any* hero visual, not as a literal transcription of what the mock-up card used to show — which is exactly why swapping the card under them for a photo cost nothing at the time, and also why removing them later (see the `lg`-only accent layer paragraph above) needed no further cleanup elsewhere: they were never tied to anything but their own `<div>`.
...
**Le titre et le sous-titre du hero ont été réécrits à la demande explicite de l'utilisateur.** Le `<h1>` était scindé en deux morceaux (« Logiciel de gestion — » en texte plein, puis « pour tous les professionnels des services. » dans un double `<span className="relative">…<span className="relative z-10">` vestige d'une décoration de soulignement déjà retirée — voir plus haut, « The decorative underline bar… ») ; c'est maintenant une phrase unique, sans tiret ni span imbriqué, ces derniers ne servant plus à rien depuis que la décoration qu'ils portaient est partie. Le sous-titre (un seul `<p>`, « Suivi du temps, missions, échéances fiscales… ») est devenu trois lignes dans un seul bloc `Reveal` (une seule animation d'entrée pour les trois, puisqu'elles forment un seul message) : une accroche en gras navy (« Gérez mieux, facturez plus, gagnez en rentabilité »), une ligne d'audience plus petite mais toujours en semi-gras (« Pour les comptables, auditeurs, fiscalistes… »), puis le corps du sous-titre d'origine reformulé (« Centralisez vos missions, pilotez vos équipes… »).

**Le `<h1>` a été réécrit une seconde fois, avec deux groupes de mots mis en couleur — puis corrigé une troisième fois sur le découpage exact de ces groupes.** « Logiciel de gestion pour tous les professionnels des services. » est devenu « Le premier logiciel tunisien conçu exclusivement pour les professionnels des services. », avec « premier logiciel » et « professionnels des services. » (point final compris) chacun dans un `<span style={{ color: '#08A4A1' }}>` — un premier essai avait coloré trois mots séparés (`logiciel`/`exclusivement`/`services`), corrigé à la demande explicite de l'utilisateur pour regrouper les mots en deux blocs précis et laisser « Le », « tunisien conçu exclusivement pour les » en couleur par défaut. Un style en ligne plutôt qu'une classe Tailwind arbitraire (`text-[#08A4A1]`), parce que `#08A4A1` est une teinte donnée par l'utilisateur pour ce rendu précis, distincte de `--color-turquoise` (`#00B3A6`) — les deux sont proches à l'œil mais ne sont pas la même valeur, et réutiliser le token de la charte aurait discrètement changé la couleur demandée ; une couleur qui n'a pas vocation à devenir un token réutilisé n'a pas besoin d'entrer dans la feuille de classes. Les deux paragraphes du sous-titre (« Pour les comptables… » et « Centralisez vos missions… ») sont passés à `font-light` — le premier était `font-semibold`, le second n'avait aucune classe de graisse (donc `font-normal` par défaut) — toujours à la demande explicite de l'utilisateur (« thin »). L'accroche « Gérez mieux, facturez plus, gagnez en rentabilité » reste `font-bold`, inchangée : seuls les deux paragraphes qui la suivent devaient s'alléger, pas elle.
...
**Le bandeau « Ils nous font confiance » ([ClientLogos.tsx](src/components/landing/ClientLogos.tsx)) porte les logos réels que le cabinet a remis, quatre à ce jour** — IQRATIC, BUSINESS ECO, TechITEasy, ACCORD Expertise Comptable. Les fichiers se déposent dans `public/logos/clients/` (servi sur `/logos/clients/…`, comme les logos CNSS/ANETI/TEJ déjà en place) et se déclarent dans `CLIENT_LOGOS`. Une entrée sans `src` — ou dont le fichier est introuvable, `onError` reprenant la main — s'affiche en **emplacement vide identifié**, jamais en icône d'image cassée : une faute de frappe dans un nom de fichier se voit tout de suite. Même principe que les avis retirés ci-dessus : afficher le logo d'une entreprise qui n'est pas cliente lui fait dire qu'elle vous recommande — c'est aussi ce qui a fait remplacer purement et simplement une première liste de 24 logos par ces quatre-là plutôt que de les cumuler : seuls ces quatre clients avaient effectivement remis leur logo à cette date. Les logos s'affichent **en couleur**, pas en nuances de gris : la moitié d'entre eux sont dans des bleus et des verts clairs qui, désaturés, deviennent illisibles sur fond blanc (comparé à l'écran avant de trancher). Le bandeau **remplace** l'ancien ruban navy des noms de modules, qui occupait cette place — le ruban défilant (`landing-marquee`/`landingMarquee`) et sa pause au survol restent à ce bandeau-là ; `landingMarqueeBack`/`landingStarPop`, propres au carrousel d'avis, sont partis avec lui.
...
**Cash's, Tâches' and RH's sub-tabs each carry their own colour, matched on the table header (or standalone total) of the content they show** — so a glance at either the active tab or the table underneath says which subview you're in without reading the label. Cash (`CASH_TAB_COLOR` in [CashManagement.tsx](src/components/cash/CashManagement.tsx)) is `documents` blue, `reglements` emerald, `journal` violet. Tâches (`TAB_COLOR` in [TaskSubviews.tsx](src/components/TaskSubviews.tsx)) is `chrono` sky, `planned` amber, `assigned` violet, `delegatedByMe` emerald — the same record also colours the `border-l-4` accent on `AssignmentList`'s and `DelegatedByMeList`'s cards. RH is `leaves` indigo, `absences` rose, `attendance` orange, `loans` teal, `advances` cyan, `holidays` fuchsia, `paie` blue (tab-bar only for `holidays` — [HolidaysTab.tsx](src/components/hr/HolidaysTab.tsx) is a card list with no `<thead>` to match); each of the other six colours is also applied to that tab's own `<thead>` in [LeavesTab.tsx](src/components/hr/LeavesTab.tsx)/[AbsencesTab.tsx](src/components/hr/AbsencesTab.tsx)/[AttendanceTab.tsx](src/components/hr/AttendanceTab.tsx)/[LoansTab.tsx](src/components/hr/LoansTab.tsx)/[AdvancesTab.tsx](src/components/hr/AdvancesTab.tsx)/[PayrollManagement.tsx](src/components/payroll/PayrollManagement.tsx) (`bg-{color}-50`/`text-{color}-700`, scoped to the header row only — the body stays neutral). None of these reuse the reserved `run`/`done`/`pause`/`late`/`admin`/`collab` tokens.

**`paie` was originally `purple`, indistinguishable at a glance from `holidays`' `fuchsia`** — both read as the same violet, so the two tabs next to each other in the bar gave no colour signal at all despite the whole point of this scheme being "glance at the tab, know the subview." Corrected to `blue` at the user's explicit request ("les deux subview bel violet, lzm ikoun different"), and `PayrollManagement.tsx`'s own `<thead>` (previously plain `bg-gray-50`/`text-gray-500`, never colour-matched to its tab even before the purple/fuchsia collision) was brought into the same `bg-blue-50`/`text-blue-700` pattern the other five tabs already followed, closing that gap at the same time.
