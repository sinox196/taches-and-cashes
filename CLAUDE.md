...
**`POST /api/services/import`** crée missions et types en masse depuis le tableur du cabinet ([ImportMissionsModal.tsx](src/components/missions/ImportMissionsModal.tsx), [parseMissionsExcel.ts](src/components/missions/parseMissionsExcel.ts)). Le fichier est lu **dans le navigateur** (SheetJS, importé dynamiquement), comme l'import des clients : le serveur ne reçoit que la liste relue et confirmée à l'écran, donc pas de middleware d'upload. Le parseur est volontairement permissif — mission en colonne A, types dans les colonnes suivantes, une première cellule vide rattachant la ligne à la mission précédente, ligne d'en-tête reconnue si elle est là — parce que le cabinet a déjà ses feuilles et qu'elles ne suivent pas un format qu'on aurait choisi. Une seule exigence dure : au moins une mission. `codepage: 65001` force l'UTF-8 pour un CSV sans BOM, le pendant du BOM que l'export CSV écrit à l'autre bout.

`applyMissionCatalogue()` est le point d'entrée commun de l'import et de la copie du catalogue de secteur — donc « ce fichier a-t-il déjà été importé » et « cette entreprise a-t-elle déjà ce modèle » se répondent pareil. Il est **strictement additif** : une mission déjà présente n'est pas recréée (elle est comptée `missionsSkipped`), mais ses types manquants lui sont ajoutés ; rien n'est renommé ni supprimé, et rejouer le même fichier ne change rien.

**Le catalogue par défaut d'un secteur** vit dans `sectorMissions`, une collection **globale** (comme `companies` et `orders`, sans `companyId` — la purge d'un tenant ne la touche donc pas). L'exploitant de la plateforme la remplit en cochant « définir comme catalogue par défaut du secteur » au moment d'un import : cette case n'apparaît que pour lui et le serveur revérifie `isPlatformAdmin`, parce que le choix engage **toutes** les entreprises du secteur, pas seulement la sienne.

`seedSectorMissions()` en donne **une copie figée** à chaque entreprise du secteur — même règle que le modèle de ressource affecté à un client : corriger le modèle ne réécrit pas ce qu'une entreprise a déjà adapté. Appelée à l'inscription *et* à la connexion : les entreprises déjà en base au moment du premier import doivent le recevoir elles aussi, et la connexion est le seul moment où l'on tient déjà la fiche entreprise en main. `sectorMissionsSeededAt` la rend définitive — sans ce drapeau, une mission délibérément supprimée reviendrait à chaque connexion. Le drapeau n'est **pas** posé tant que le catalogue est vide, sinon les entreprises connectées avant le premier import ne le recevraient jamais. C'est une `function` déclarée et non un `const` : la route de connexion, enregistrée bien plus haut dans `startServer()`, l'appelle — même raison que `notify()`.
