...
**The overtime alert asks at most once every 2 hours per task** ([App.tsx](src/App.tsx)). Once a running task passes `OVERTIME_THRESHOLD_SECONDS` (2h) it prompts "Toujours sur cette tâche ?"; no answer within the 2-minute grace pauses it automatically. The gap between prompts is **wall-clock time, persisted** in `localStorage.overtime_prompted_at` (entry id → epoch ms of the last prompt, pruned after 7 days), and the stamp is written when the popup is *shown*, not when it is answered — so a reload while it is open does not bring it straight back.

It used to count 2h boundaries crossed in a `useRef`, and that was wrong twice over: the ref died on every remount, and `dureeSeconds` is **accumulated, not continuous**, so a task that had ever passed 2h was past it forever. Between them, every single page load re-fired the popup immediately on any long-running task. Don't reintroduce a memory-only record here — the whole feature is a promise about elapsed real time, so it has to survive a refresh.
