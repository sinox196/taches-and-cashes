...
**Le `<h1>` a été réécrit une seconde fois, avec trois mots mis en couleur.** « Logiciel de gestion pour tous les professionnels des services. » est devenu « Le premier **logiciel** tunisien conçu **exclusivement** pour les professionnels des **services**. » — trois mots (`logiciel`, `exclusivement`, `services`) chacun dans un `<span style={{ color: '#08A4A1' }}>` plutôt qu'une classe Tailwind, parce que `#08A4A1` est une teinte donnée par l'utilisateur pour ce rendu précis, distincte de `--color-turquoise` (`#00B3A6`) — les deux sont proches à l'œil mais ne sont pas la même valeur, et réutiliser le token de la charte aurait discrètement changé la couleur demandée. Un style en ligne plutôt qu'une classe Tailwind arbitraire (`text-[#08A4A1]`) pour la même raison qu'ailleurs dans ce fichier : une couleur qui n'a pas vocation à devenir un token réutilisé n'a pas besoin d'entrer dans la feuille de classes. Les deux paragraphes du sous-titre (« Pour les comptables… » et « Centralisez vos missions… ») sont passés à `font-light` — le premier était `font-semibold`, le second n'avait aucune classe de graisse (donc `font-normal` par défaut) — toujours à la demande explicite de l'utilisateur (« thin »). L'accroche « Gérez mieux, facturez plus, gagnez en rentabilité » reste `font-bold`, inchangée : seuls les deux paragraphes qui la suivent devaient s'alléger, pas elle.
