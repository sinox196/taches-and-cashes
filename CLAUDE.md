...
**A second collapsible section, "Paramètres de la paie", sits right below "Gestion des paies"** on the same form, same chevron idiom, own `useState`, collapsed by default, same `formRole !== CLIENT_ROLE` gate — the six déductions communes inputs above (`paieMarie` as a checkbox; `paieEnfantsInfirmes`, `paieEnfantsEtudiants`, `paieParentsACharge` as a 0/1/2 `<select>`, `paieAssuranceVie`, `paieCEA` as number inputs), each carrying its exact rule/cap as helper text so the admin never has to guess what a field does from its name alone. `nombreEnfants` stays in "Gestion des paies" above rather than being duplicated here — a note under the new section says so. The modal widened again, `max-w-2xl` → `max-w-4xl`, to keep the two-column grid legible with two stacked payroll sections instead of one.
