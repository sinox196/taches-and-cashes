...
L'assignation reste **en attente** — dans **Tâches**, sous ses deux sous-vues [TaskSubviews.tsx](src/components/TaskSubviews.tsx) : « Tâches planifiées » (ce que vous vous êtes réservé) et « Tâches assignées » (ce qu'on vous a délégué). Elles vivaient dans une carte du tableau de bord (`AssignedTasksCard`, montée sur les *deux* tableaux de bord parce que `STAFF_ROLES` déborde des rôles qui voient MyDashboard) ; elles sont maintenant là où on les démarre, puisque démarrer une tâche crée une entrée de pointage — c'est-à-dire l'onglet d'à côté, et non plus une traversée « tableau de bord → Tâches ». La carte a été **supprimée**, pas laissée en double : deux endroits qui listent les mêmes tâches finissent par n'en montrer qu'un des deux à jour.
