...
**Déductions communes cover only Marié(e) and Nombre d'enfants (capped at 4), not the barème's full family-quotient table.** The cahier des charges' own deduction table also has parents à charge, assurance vie, compte épargne en actions, enfants handicapés (uncapped) and enfants étudiants non boursiers (capped at 4, on top of the ordinary four) — none of that data exists anywhere on a user's fiche, only `situationFamiliale` (free text, matched case-insensitively for "mari…") and `nombreEnfants` (a plain count). Building those five extra deductions would mean inventing data the form doesn't collect. Deliberately deferred, like the rest of this app's "not built, not guessed" sections — not a bug, a documented scope line.

**The twelve fields live on the Équipe user form**, grouped in a collapsible "Gestion des paies" section (chevron toggle, own `useState`, collapsed by default — the same idiom the dashboard's five collapsible cards and the permission groups just below it already use) gated `formRole !== CLIENT_ROLE`: matricule, n° CIN, n° CNSS, qualification, département, banque/poste, numéro de compte, situation familiale, nombre d'enfants, catégorie, échelon, salaire/heure. All twelve are purely a payroll dossier — none of them feed `employerHourlyRate()` or the pointage gate, only the bulletin. The modal widened from `max-w-md` to `max-w-2xl` to hold the two-column grid these fields need — a single column would have made an already long form scroll far past what a password/role/coût-employeur form used to require.

**A bulletin snapshots the employee's payroll identity at generation time** — matricule, CIN, CNSS, qualification, département, banque, compte, situation familiale, nombre d'enfants, catégorie, échelon, salHeure — the same "copie figée" rule the mission/task-type snapshot on time entries and the client-flag snapshot on invoices already follow: correcting an employee's CIN in Équipe next year must not silently rewrite a bulletin already handed out. Editing a saved bulletin (`PUT /api/payslips/:id`) only ever touches the editable gains/rates, never re-reads the employee row — a bulletin's year/month/employee are fixed at creation, like an invoice's number; regenerate rather than reassign. Creating a second bulletin for the same (employee, year, month) is refused server-side.

**`GET /api/payroll/employees` is a liste blanche projection**, not the full user row — id, username, role, and the payroll-relevant fields only, never permissions or the rest of the coût-employeur block, so a `VIEW_PAYROLL` holder without `MANAGE_USERS` can still pick an employee and see their payroll identity without being handed the whole Équipe fiche. `GET /api/payroll/company` reuses the exact same issuer identity as Cash (`companyBlock(getSettings())`) rather than a second copy — it is company identity, not invoicing-specific — behind its own `VIEW_PAYROLL` gate so a payroll-only viewer isn't forced through `VIEW_CASH` to print a bulletin's header.

**[payslipPdf.ts](src/components/payroll/payslipPdf.ts) draws the bulletin with jsPDF text primitives**, the same "real vector text, one renderer for download and print" rule [invoicePdf.ts](src/components/cash/invoicePdf.ts) already follows — reusing its `CompanyBlock` type and the print-via-hidden-iframe pattern rather than inventing a second one. **It follows the cabinet's markdown template ("Fiche_de_Paie_Zainab_Ben_Attiya.md") section by section**, not a restyled adaptation of the earlier PDF sample the cabinet had also sent — the two disagree on structure (a single top title vs. the company name + "Période du" as the masthead and "BULLETIN DE PAIE" as a mid-document heading; one boxed Net à payer/Signature area vs. three separate lines), and the markdown is the later, more explicit of the two. The two tables keep the same plain-black, fully-bordered Excel-style treatment either way — every cell its own bordered rect via a shared `box()` helper, never a separate divider line drawn on top of already-bordered cells (an earlier version did that and it crossed straight through text) — but the document otherwise reads top to bottom exactly like the markdown: masthead, "Informations Salarié" (a plain symmetric label/value grid now, six rows, no merged rows or spanning cells like the older PDF-derived layout had), a rule, "BULLETIN DE PAIE", the rubriques table, "SALAIRE NET A PAYER" as its own heading with "Net à payer" and "Montant en lettres" as separate lines beneath it, "Compteurs et Temps de Travail", the disclaimer, and "Signature & cachet" as a closing line rather than a box.

**The rubriques table gained a fifth column, BASE / NOMBRE, between RUBRIQUES and TAUX** — a quantity (hours, days) distinct from a rate/percentage, per the markdown's own header row. Populated only where a quantity actually exists in the data this app tracks: `nbHeures` for SALAIRE DE BASE, `joursFeries` for JOURS FÉRIÉS — both otherwise-informational counters (see below) reused here since the template shows the identical number in both places. Every other row leaves it blank rather than inventing a quantity the app has no source for (the primes' own BASE/NOMBRE and TAUX cells in the cabinet's original worked example are numbers this app has no formula to reproduce — see the note on primes below). TAUX itself holds plain numbers as the model types them (`176`, `6.99`, `0.5`) — **never run through `money()`**, which forces a comma and three decimals and would turn "6.99" into "6,990"; only GAINS/RETENUES are money-formatted. Every row prints at the same plain weight — no bolding on intermediate lines — except the header row and SALAIRE BRUT/BRUT IMPOSABLE/NET, matching the markdown's own bold cells.

**Nb Heures, N° Heures, J.Congés, J.Absences and Solde Congé are purely informational** — admin-typed numbers printed on the "Compteurs et Temps de Travail" table, exactly as the template has them, entering no calculation. `Jours fériés` is the one exception: the same number both prices the "Jours fériés" gains line and prints in that table, since the template shows the identical figure in both places.
