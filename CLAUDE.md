...
`seedSectorMissions()` en donne **une copie figée** à chaque entreprise du secteur (`SECTOR_MISSIONS[secteur]`, `CABINET` seul étant servi — c'est du contenu de cabinet comptable). Même règle que le modèle de ressource affecté à un client : ce que l'entreprise en fait ensuite lui appartient. Appelée à l'inscription *et* à la connexion, parce que les entreprises déjà en base doivent le recevoir aussi et que la connexion est le seul moment où l'on tient déjà la fiche entreprise en main. `sectorMissionsSeededAt` la rend définitive — sans ce drapeau, une mission délibérément supprimée reviendrait à chaque connexion. C'est une `function` déclarée et non un `const` : la route de connexion, enregistrée bien plus haut dans `startServer()`, l'appelle — même raison que `notify()`. `applyMissionCatalogue()` pose le catalogue sans jamais créer de doublon, et est strictement additif.
