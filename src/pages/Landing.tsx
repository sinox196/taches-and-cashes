...
import {
  ...
  Compass, Clock, AlertTriangle, ArrowDown, Search, HelpCircle, Headphones,
  MessageCircle, Sparkles, Video, Download, ChevronDown, BookOpen, CheckCircle2,
} from 'lucide-react';
...
/**
 * Each entry mirrors a real chapter of the user guide (public/guide —
 * downloadable from this page), condensed to what actually matters for a
 * quick answer rather than the document's full walkthrough prose. Content
 * only, never a claim this app doesn't back: no fabricated screenshot, no
 * video, no changelog entry invented to fill a card — see the "Bientôt
 * disponible" cards below for the two resource types that don't exist yet.
 */
const GUIDE_CHAPTERS: { title: string; summary: string; icon: React.ReactNode }[] = [
  {
...
export const Landing: React.FC<LandingProps> = ({ onLogin }) => {
  ...
  // Support page's own local state — search filters the FAQ and the guide
  // chapters in place rather than routing anywhere (this page has no router
  // to route to), "Voir toutes les FAQ" reveals the rest of the list instead
  // of hiding a fixed six forever, and each guide chapter opens on its own.
  const [supportQuery, setSupportQuery] = useState('');
  const [showAllFaq, setShowAllFaq] = useState(false);
  const [openGuideChapters, setOpenGuideChapters] = useState<Set<string>>(new Set());
  const toggleGuideChapter = (title: string) => {
    setOpenGuideChapters(prev => {
      const next = new Set(prev);
      if (next.has(title)) next.delete(title); else next.add(title);
      return next;
    });
  };
  const foldSupport = (v: string) => v.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
  const supportQueryFolded = foldSupport(supportQuery.trim());
  const filteredFaq = supportQueryFolded
    ? FAQ_ITEMS.filter(f => foldSupport(f.q).includes(supportQueryFolded) || foldSupport(f.a).includes(supportQueryFolded))
    : (showAllFaq ? FAQ_ITEMS : FAQ_ITEMS.slice(0, 6));
  const filteredGuideChapters = supportQueryFolded
    ? GUIDE_CHAPTERS.filter(c => foldSupport(c.title).includes(supportQueryFolded) || foldSupport(c.summary).includes(supportQueryFolded))
    : GUIDE_CHAPTERS;
  ...
  return (
    <div className="min-h-screen bg-white font-sans antialiased text-gray-900">
      ...
      {view === 'home' ? (
        ...
        <>
          {/* SUPPORT HERO — split like the home hero (copy left, a visual
              right), unlike Tarifs/À propos's centered single column: this
              is the one view the brief asks to carry an illustration, so it
              earns a different hero shape rather than reusing theirs. */}
          <section className="pt-[88px] px-6 sm:px-10 pb-16 bg-[linear-gradient(180deg,#FBFCFD_0%,#F2F4F7_100%)] overflow-hidden">
              ...
              <div style={{ flex: '1 1 460px', minWidth: 300 }}>
                ...
                <Reveal delay={150}>
                  <p className="mt-[18px] text-[16.5px] text-[#5B6472] leading-[1.6] max-w-[480px]">
                    Trouvez rapidement des réponses à vos questions, accédez à nos guides et contactez notre équipe support.
                  </p>
                </Reveal>

                <Reveal delay={210}>
                  <div className="mt-8 relative max-w-[440px]">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-[#8A93A0]" />
                    <input
                      type="text"
                      value={supportQuery}
                      onChange={e => setSupportQuery(e.target.value)}
                      placeholder="Rechercher une aide, un guide, une question…"
                      className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-[#E6E9EE] bg-white text-[14.5px] text-navy placeholder:text-[#8A93A0] shadow-[0_10px_24px_-14px_rgba(13,27,42,0.18)] focus:outline-none focus:ring-2 focus:ring-turquoise/40 focus:border-turquoise transition-shadow"
                    />
                  </div>
                </Reveal>
                ...
                <Reveal delay={260}>
                  <div className="mt-4 flex flex-wrap gap-2.5">
                    <button onClick={() => scrollToId('support-resources')} className="px-4 py-2 rounded-full bg-white border border-[#E6E9EE] text-[13px] font-semibold text-[#3D4655] hover:border-turquoise/50 hover:text-navy transition-colors">Guides</button>
                    <button onClick={() => scrollToId('support-faq')} className="px-4 py-2 rounded-full bg-white border border-[#E6E9EE] text-[13px] font-semibold text-[#3D4655] hover:border-turquoise/50 hover:text-navy transition-colors">FAQ</button>
                    <button onClick={() => scrollToId('support-contact')} className="px-4 py-2 rounded-full bg-white border border-[#E6E9EE] text-[13px] font-semibold text-[#3D4655] hover:border-turquoise/50 hover:text-navy transition-colors">Assistance</button>
                  </div>
                </Reveal>
              </div>

              {/* A conseillère support illustration, built the way every
                  other maquette on this page is — divs and CSS shapes, not
                  an exported image, so it stays crisp at any zoom and a
                  future copy tweak never has to touch a design file. Flat,
                  brand-coloured, no attempt at a literal face: an abstract
                  navy silhouette wearing a turquoise headset reads clearly
                  at a glance without risking an uncanny hand-coded portrait. */}
              <Reveal direction="left" style={{ flex: '1 1 320px', minWidth: 260 }} className="w-full">
                <div className="relative mx-auto max-w-[360px] bg-white rounded-[28px] p-10 overflow-hidden" style={{ boxShadow: '0 30px 70px -30px rgba(13,27,42,0.22)' }}>
                  <div
                    aria-hidden
                  ...
                  />
                  <div className="relative mx-auto" style={{ width: 220, height: 230 }}>
                    {/* torso */}
                    ...
                    {/* face — two eyes and a soft smile, subtle rather than literal */}
                  ...
                  {/* floating status cards — same grammar as the À propos
                      illustration: a pulsing dot for "live", a plain fact
                      card for the other. Both say something real: the dot
                      isn't decorative, and 24h is the number the contact
                      section below states too. */}
                  <div className="absolute top-6 right-6 bg-white rounded-2xl px-3.5 py-2.5" style={{ boxShadow: '0 14px 26px -10px rgba(13,27,42,0.16)' }}>
                    <div className="flex items-center gap-1.5">
                      <span className="w-[6px] h-[6px] rounded-full bg-[#22C55E] shrink-0 animate-[landingPulseDot_2s_ease-in-out_infinite]" />
                      <span className="text-[11.5px] font-extrabold text-navy leading-tight whitespace-nowrap">Support en ligne</span>
                    </div>
                  </div>
                  <div className="absolute bottom-7 left-4 bg-white rounded-2xl px-3.5 py-2.5 flex items-center gap-2" style={{ boxShadow: '0 14px 26px -10px rgba(13,27,42,0.16)' }}>
          ...
          {/* RESSOURCES D'AIDE */}
          <section id="support-resources" className="py-20 px-6 sm:px-10 bg-white">
            <div className="max-w-[1100px] mx-auto">
              <Reveal className="text-center max-w-[640px] mx-auto">
                <div className="inline-flex px-3.5 py-1.5 bg-[#F2F4F7] rounded-full text-[12px] font-bold tracking-[0.06em] uppercase text-[#00857C]">Ressources d'aide</div>
                <h2 className="mt-[18px] text-[26px] sm:text-[32px] font-extrabold text-navy tracking-[-0.01em]">De quoi avez-vous besoin ?</h2>
              </Reveal>

              <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                  { title: 'Guides utilisateurs', desc: 'Découvrez les guides pas à pas pour utiliser les différents modules de la plateforme.', icon: <BookOpen className="w-5 h-5" />, iconBg: '#E3F7F5', iconColor: '#00857C', onClick: () => scrollToId('support-guide'), soon: false },
                  { title: 'FAQ', desc: 'Trouvez rapidement les réponses aux questions fréquentes.', icon: <HelpCircle className="w-5 h-5" />, iconBg: '#E9ECFE', iconColor: '#3B52C4', onClick: () => scrollToId('support-faq'), soon: false },
                  { title: 'Tutoriels vidéo', desc: 'Apprenez à utiliser les fonctionnalités principales de Tâches & Cash en quelques minutes.', icon: <Video className="w-5 h-5" />, iconBg: '#FFF3DE', iconColor: '#C98A1B', onClick: undefined, soon: true },
                  { title: 'Nouveautés', desc: 'Découvrez les dernières évolutions et mises à jour de la plateforme.', icon: <Sparkles className="w-5 h-5" />, iconBg: '#FDEBEF', iconColor: '#C2416B', onClick: undefined, soon: true },
                ].map((card, i) => (
                  <Reveal key={card.title} delay={i * 70} className="h-full">
                    <div
                      onClick={card.onClick}
                      className={`landing-shine group relative h-full bg-white rounded-[18px] border border-[#E6E9EE] p-6 transition-all duration-300 ${card.onClick ? 'cursor-pointer hover:-translate-y-1.5 hover:border-turquoise/45 hover:shadow-[0_18px_38px_rgba(13,27,42,0.10)]' : 'opacity-80'}`}
                    >
                      {card.soon && (
                        <span className="absolute top-4 right-4 px-2 py-[3px] rounded-full bg-[#F2F4F7] text-[10px] font-bold text-[#8A93A0]">Bientôt disponible</span>
                      )}
                      <div className="w-[42px] h-[42px] rounded-xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110" style={{ background: card.iconBg, color: card.iconColor }}>
                        {card.icon}
                      </div>
                      <div className="text-[15px] font-bold text-navy mt-4">{card.title}</div>
                      <p className="text-[13.5px] leading-[1.55] text-[#5B6472] mt-2">{card.desc}</p>
                    </div>
                  </Reveal>
                ))}
              </div>

              {/* LE GUIDE, MODULE PAR MODULE — le contenu réel du guide
                  utilisateur (voir public/guide), condensé en réponses
                  courtes plutôt que recopié tel quel : la même structure de
                  chapitres, pensée pour se lire à l'écran. */}
              <div id="support-guide" className="mt-16 scroll-mt-6">
                <div className="flex flex-wrap items-end justify-between gap-4">
                  <div>
                    <h3 className="text-[20px] sm:text-[22px] font-extrabold text-navy tracking-[-0.01em]">Le guide, module par module</h3>
                    <p className="mt-1.5 text-[14px] text-[#5B6472]">Cliquez un module pour dérouler l'essentiel à savoir.</p>
                  </div>
                  <a
                    href="/guide/Guide-Taches-et-Cash.docx"
                    download
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#F2F4F7] text-[13.5px] font-bold text-navy hover:bg-navy hover:text-white transition-colors whitespace-nowrap"
                  >
                ...
                <div className="mt-6 divide-y divide-[#E6E9EE] border border-[#E6E9EE] rounded-[18px] overflow-hidden bg-white">
                  {filteredGuideChapters.length === 0 ? (
                    <p className="px-6 py-8 text-center text-[14px] text-[#5B6472]">Aucun module ne correspond à « {supportQuery} ».</p>
                  ) : filteredGuideChapters.map(chapter => {
                    const isOpen = openGuideChapters.has(chapter.title);
                    return (
                      <div key={chapter.title}>
                        <button
                          onClick={() => toggleGuideChapter(chapter.title)}
                        ...
                        >
                          ...
                          <ChevronDown className={`w-[18px] h-[18px] text-[#8A93A0] shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-5 sm:px-6 pb-5 pl-[62px] sm:pl-[66px]">
                            <p className="text-[13.5px] leading-[1.65] text-[#5B6472] max-w-[720px]">{chapter.summary}</p>
                          </div>
                        )}
                      </div>
                    );
          ...
          <section id="support-faq" className="py-20 px-6 sm:px-10 bg-[#F2F4F7] scroll-mt-6">
            <div className="max-w-[820px] mx-auto">
              <Reveal className="text-center max-w-[640px] mx-auto">
                <div className="inline-flex px-3.5 py-1.5 bg-white border border-[#E6E9EE] rounded-full text-[12px] font-bold tracking-[0.06em] uppercase text-[#00857C]">Questions fréquentes</div>
                <h2 className="mt-[18px] text-[26px] sm:text-[32px] font-extrabold text-navy tracking-[-0.01em]">Les réponses les plus demandées</h2>
              </Reveal>

              {supportQueryFolded && (
                <p className="mt-6 text-center text-[13.5px] text-[#5B6472]">
                  {filteredFaq.length > 0
                    ? `${filteredFaq.length} question${filteredFaq.length > 1 ? 's' : ''} pour « ${supportQuery} »`
                    : `Aucune question ne correspond à « ${supportQuery} ».`}
                </p>
              )}

              <div className="mt-10 space-y-3.5">
                {filteredFaq.map(item => (
                  <div key={item.q} className="bg-white rounded-[16px] border border-[#E6E9EE] px-6 py-5">
                    <div className="flex items-start gap-3">
                      <CheckCircle2 className="w-[18px] h-[18px] text-turquoise shrink-0 mt-0.5" />
                      <div>
                        <p className="text-[14.5px] font-bold text-navy">{item.q}</p>
                        <p className="mt-1.5 text-[13.5px] leading-[1.6] text-[#5B6472]">{item.a}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {!supportQueryFolded && !showAllFaq && FAQ_ITEMS.length > 6 && (
                <div className="mt-8 text-center">
                  <button
                    onClick={() => setShowAllFaq(true)}
                    className="px-6 py-3 rounded-xl text-[14px] font-bold text-navy border-[1.5px] border-[#E6E9EE] bg-white hover:border-turquoise/50 transition-colors"
                  >
                    Voir toutes les FAQ
                  </button>
                </div>
          ...
          <section id="support-contact" className="py-20 px-6 sm:px-10 bg-white scroll-mt-6">
            <div className="max-w-[1000px] mx-auto">
              <Reveal className="text-center max-w-[640px] mx-auto">
                <div className="inline-flex px-3.5 py-1.5 bg-[#F2F4F7] rounded-full text-[12px] font-bold tracking-[0.06em] uppercase text-[#00857C]">Besoin d'aide ?</div>
                <h2 className="mt-[18px] text-[26px] sm:text-[32px] font-extrabold text-navy tracking-[-0.01em]">Notre équipe support est disponible pour vous accompagner</h2>
              </Reveal>

              <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-5">
                <a href={`tel:${CONTACT_PHONE.replace(/\s/g, '')}`} className="landing-shine group bg-white rounded-[18px] border border-[#E6E9EE] p-7 hover:-translate-y-1.5 hover:border-turquoise/45 hover:shadow-[0_18px_38px_rgba(13,27,42,0.10)] transition-all duration-300">
                  ...
                  <div className="text-[15px] font-bold text-navy mt-4">Par téléphone</div>
                  <p className="text-[14.5px] text-[#3D4655] mt-2 font-semibold">{CONTACT_PHONE}</p>
                </a>
                <a href={`mailto:${CONTACT_EMAIL}`} className="landing-shine group bg-white rounded-[18px] border border-[#E6E9EE] p-7 hover:-translate-y-1.5 hover:border-turquoise/45 hover:shadow-[0_18px_38px_rgba(13,27,42,0.10)] transition-all duration-300">
                  ...
                  <div className="text-[15px] font-bold text-navy mt-4">Par email</div>
                  <p className="text-[14.5px] text-[#3D4655] mt-2 font-semibold break-all">{CONTACT_EMAIL}</p>
                  <p className="text-[12.5px] text-[#8A93A0] mt-1">Réponse sous 24h</p>
                </a>
                <a href={CONTACT_WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="landing-shine group bg-white rounded-[18px] border border-[#E6E9EE] p-7 hover:-translate-y-1.5 hover:border-turquoise/45 hover:shadow-[0_18px_38px_rgba(13,27,42,0.10)] transition-all duration-300">
                  <div className="w-[46px] h-[46px] rounded-xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110" style={{ background: '#E9F9EE', color: '#1C9A4B' }}>
                    <MessageCircle className="w-[22px] h-[22px]" />
                  </div>
                  <div className="text-[15px] font-bold text-navy mt-4">Chat en ligne</div>
                  <p className="text-[13.5px] text-[#5B6472] mt-2 leading-[1.5]">Discutez avec notre équipe support en temps réel, sur WhatsApp.</p>
                </a>
              </div>
            </div>
          </section>
