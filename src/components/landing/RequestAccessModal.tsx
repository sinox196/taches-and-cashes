...
/**
 * Le libellé de la carte cliquée -> le code que le serveur attend. Dérivé de
 * [plans.ts](../../constants/plans.ts) pour qu'ajouter ou renommer une offre
 * reste une seule édition : une table écrite à la main ici finirait par ne
 * plus reconnaître un pack et retomberait silencieusement sur le formulaire
 * de contact.
 */
const PLAN_CODES: Record<string, string> =
  Object.fromEntries(SELLABLE_PLANS.map(p => [p.label, p.id]));

/**
 * Two different flows behind one modal, picked by which pricing card was
 * clicked:
 *
 * - The standard packs (see plans.ts) are a real
 ...
 *   manually confirmed.
 * - "Sur mesure" (beyond the largest pack) stays a lead-capture request — a custom deal is
