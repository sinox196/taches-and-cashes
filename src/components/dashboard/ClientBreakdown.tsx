              ...
              return (
                <React.Fragment key={key}>
                  ...
                  >
                    ...
                    {/* The client's running ledger — lifetime, not scoped to the
                        dashboard's date filter (same figures as the Clients page). */}
                    {isAdmin && (
                      <td className="px-4 py-3 text-right font-mono text-gray-700">
                        {formatCostTND(client.soldeAnterieur ?? 0)}
                      </td>
                    )}
                    {isAdmin && (
                    ...
                    {isAdmin && (
                      <td className="px-4 py-3 text-right">
                        <span className={`font-semibold ${
                          Math.abs(client.resteAPayer ?? 0) < 0.001
                            ? 'text-emerald-700'
                            : (client.resteAPayer ?? 0) < 0 ? 'text-amber-700' : 'text-gray-900'
                        }`}>
                          {Math.abs(client.resteAPayer ?? 0) < 0.001 ? 'Soldé' : formatCostTND(client.resteAPayer ?? 0)}
                        </span>
                      </td>
                    )}
                  </tr>
