...
async function startServer() {
  ...
  const notifySeatOverageIfNeeded = async (company: any, usedBefore: number, role: string, newUsername: string) => {
    ...
    // Le montant réellement dû pour ce nombre de comptes — même
    // `planPriceForSeats()` que la page Tarifs et la confirmation de
    // paiement, jamais un second calcul qui pourrait diverger. `usedAfter`
    // (pas `usedBefore`) : c'est ce que l'entreprise doit payer maintenant
    // que ce compte existe, pas ce qu'elle devait avant.
    const usedAfter = usedBefore + 1;
    const monthlyDue = sellablePlan ? planPriceForSeats(sellablePlan, usedAfter) : null;
    await sendMail({
      to: 'contact@taches-and-cash.com',
      ...
      html: `
        ...
        <p><strong>Date :</strong> ${formatDateFR(new Date())}</p>
        <p><strong>Total utilisateurs back-office :</strong> ${usedAfter} (${usedAfter - limit} au-delà des sièges souscrits)</p>
        ${monthlyDue !== null ? `<p><strong>Montant mensuel dû pour ce nombre d'utilisateurs :</strong> ${escapeHtml(formatDT(monthlyDue))}/mois</p>` : ''}
      `,
    });
  };
...
app.post('/api/dashboard/executive', authenticate, async (req: any, res: any) => {
  ...
  app.get('/api/invoices', authenticate, requirePermission('VIEW_CASH'), async (req: any, res: any) => {
    try {
      ...
      res.json({
        data: page, total: filtered.length, limit, offset,
